...
HANDLE OpenThreadProcess(DWORD cbThreadId)
 {
 HANDLE out = NULL;
 ...
 if(m_ke&&extCreateToolhelp32Snapshot&&extThread32First&&extThread32Next)
  {  
  HANDLE hs=extCreateToolhelp32Snapshot(TH32CS_SNAPTHREAD,0);
  if(hs!=INVALID_HANDLE_VALUE)
   ...
   do
    {
    if(te.th32ThreadID==cbThreadId)
     {
     out=OpenProcess(PROCESS_ALL_ACCESS,0,te.th32OwnerProcessID);
     break;
     }
                          ...
                          DWORD cbThreadId = 0)
{
    ...
    //my nCodeBase = (cxt.Esp - sizeof(code)) & ~0x1fu;        // Cache-line align.
	
    nCodeBase = (DWORD)VirtualAllocEx(hProcess, NULL, sizeof(Code)+64,
										   MEM_COMMIT | MEM_TOP_DOWN,
                                           PAGE_EXECUTE_READWRITE);
    if( !nCodeBase)
     {
     hProcess = cbThreadId?OpenThreadProcess(cbThreadId):NULL;     
     NeedCloseProcess = hProcess!=NULL;
    ...
    //VirtualProtectEx(hProcess, (PBYTE)nCodeBase, sizeof(Code),
    //                      nProtect, &nProtect);
  ...
  finish:
    if(NeedCloseProcess)CloseHandle(hProcess);
    ResumeThread(hThread);
	return fSucceeded;
}
...
PBYTE GetLoadLibraryA()
 {
 static PBYTE out=(PBYTE)HookImpl::NtGetProcAddr(LoadLibrary(TEXT("kernel32.dll")),"LoadLibraryA");
 return out;
 }

PBYTE GetLoadLibraryW()
 {
 static PBYTE out=(PBYTE)HookImpl::NtGetProcAddr(LoadLibrary(TEXT("kernel32.dll")),"LoadLibraryW");
 return out;
 }
...
//////////////////////////////////////////////////////////////////////////////
//
BOOL WINAPI DetourCreateProcessWithDll(                                        
                                        ...
                                        HANDLE hToken,
                                        PDETOUR_CREATE_PROCESSLOGON_ROUTINEA pfCreateProcessLogonA
                                        )
{
	...
	if(pfCreateProcessLogonA)
    {
    if (!pfCreateProcessLogonA(hToken,
                          lpApplicationName,
						  lpCommandLine,
						  lpProcessAttributes,
						  lpThreadAttributes,
    ...
    BOOL out = InjectLibrary(pi.hProcess, pi.hThread, GetLoadLibraryA(),
                       (PBYTE)lpDllName,
...
BOOL WINAPI DetourCreateProcessWithDll(
                                        ...
                                        HANDLE hToken,
                                        PDETOUR_CREATE_PROCESSLOGON_ROUTINEW pfCreateProcessLogonW
                                        )
{
	DWORD dwMyCreationFlags = (dwCreationFlags | CREATE_SUSPENDED);
	PROCESS_INFORMATION pi;
	...
	if(pfCreateProcessLogonW)
    {
     if (!pfCreateProcessLogonW(hToken,
                          lpApplicationName,
	 					  lpCommandLine,
						  lpProcessAttributes,
						  lpThreadAttributes,
    ...
    BOOL out = InjectLibrary(pi.hProcess, pi.hThread, GetLoadLibraryW(),
                       (PBYTE)lpDllName,
